<script>
	...
	import { _sendEpisodes, _getImages } from './+page';
	...
	onMount(() => {
		...
		_getImages(Object.keys(shows['episodes'])).then((response) => {
			images = response;
			console.log(images);
		});
	});
			...
			>
				<div class="flex flex-row gap-2">
					<svg
						...
						/></svg
					>
					<a href="https://www.imdb.com/title/{episode.ID}/" target="”_blank”" class="text-xl"
						>{episode.title}</a
					>
				</div>
